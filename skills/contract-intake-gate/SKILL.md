...
2. **必须全文逐处扫描，不能只看首部与落款。**主体名称不一致最常出现在正文中段的某一个条款里
   ...
   - 去掉或替换了行政区划、行业词的（`云澈信息技术（上海）有限公司` → `云澈科技（上海）有限公司`）
   - 简称与全称混用且未在正文定义简称的
   - 繁简、全半角、括号形态不一致的
...
blocks:
  - code: BLK-PARTY-INCONSISTENT
    ...
    evidence:
      part: body
      page: 6
      quote: "乙方应在 10 个工作日内向云澈科技（上海）有限公司移交全部已完成的阶段性成果"
    finding: 甲方权威名称为「云澈信息技术（上海）有限公司」，本条出现「云澈科技（上海）有限公司」，
      未在首部定义为简称，接收主体身份不明
...
contract_intake_receipt:
  ...
  remediation:                          # 补齐清单（祈使句）
    ...
    - 将 10.4 的接收主体更正为甲方全称「云澈信息技术（上海）有限公司」
