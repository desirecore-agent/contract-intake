...
test('source wiring requires the deterministic receipt writer and structural validator', async () => {
  ...
  assert.match(skill, /`valid: false`、任何路径\/schema\/parser\/runtime 工具错误或未获结果.*HOLD/s)
  assert.match(skill, /输出槽是 create-only：不得为修复结构错误重用同一路径、\s*改写回执或另造替代回执/)
  assert.match(skill, /<有效工作目录>\/<intake_id>\.receipt\.yaml/)
  assert.match(skill, /`Ls` 必须实际确认 effective cwd；`output_path` 只能是该已存在受权根内、由本次真实 `intake_id` 构成且尚不存在的唯一/)
  assert.match(skill, /路径已存在、不是绝对路径、落在保留根内，或平台返回的 output path 不等于请求的唯一目标时如实 HOLD/)
  assert.doesNotMatch(skill, /contract-review-members\/contract-intake/)
